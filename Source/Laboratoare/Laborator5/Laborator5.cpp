...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_K)) {
			projectionMatrix = glm::perspective(RADIANS(x), window->props.aspectRatio, 0.01f, 200.0f);
			x++;
		}
		...
		// Latimea / inaltimea ferestrei
		if (window->KeyHold(GLFW_KEY_M)) {
			projectionMatrix = glm::ortho(-8.f, 8.f, -4.0f, 3.f, 0.01f, 200.f);
		}

		if (window->KeyHold(GLFW_KEY_N)) {
			projectionMatrix = glm::ortho(10.f, -8.f, -4.5f, 4.5f, 0.01f, 200.f);
		}
...
void Laborator5::OnKeyPress(int key, int mods)
{
	...
	if (key == GLFW_KEY_O) {
		projectionMatrix = glm::ortho(-8.f, 8.f, -4.5f, 4.5f, 0.01f, 200.f);
	}
